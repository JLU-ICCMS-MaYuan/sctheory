...
本文档是 `quantum_geometry/` 目录的执行入口，目标是把以下两篇文献中关于石墨烯和 MgB2 的核心结果，系统地复现到本仓库中：

- `reference/2024 EPC-量子几何.pdf`
- `reference/2024 EPC-量子几何-SI.pdf`
...
本次复现的所有新增内容只放在 `quantum_geometry/` 下，不污染现有研究目录。
...
- 把 `reference/reproduce_mgb2_tba.py` 升级为完整 `px,py` 紧束缚
