...
def hasSOC(prefix):
    fname =  'tmp/' + prefix+'.save/data-file-schema.xml'
...
def isSEQ(prefix):
    fname = 'tmp/_ph0/'+str(prefix)+'.dvscf'
    if (os.path.isfile(fname)):
        lseq = True
    else:
        lseq = False
    ...
    if SEQ:
        ...
        if XML:
            ...
            os.system('cp '+prefix+'.dyn'+str(iqpt)+'.xml save/'+prefix
                      + '.dyn_q'+label+'.xml')
            if (iqpt == 1):
                os.system('cp tmp/_ph0/'+prefix+'.dvscf* save/'+prefix+'.dvscf_q'
                          + label)
                os.system('cp -r tmp/_ph0/'+prefix+'.phsave save/')
                if fc:
                    os.system('cp '+prefix+'.fc.xml save/ifc.q2r.xml')
                if PAW:
                    os.system('cp tmp/_ph0/'+prefix+'.dvscf_paw* save/'+prefix +
                              '.dvscf_paw_q'+label)
            else:
        ...
        if XML:
            ...
            os.system('cp '+prefix+'.dyn'+str(iqpt)+'.xml save/'+prefix +
                      '.dyn_q'+label+'.xml')
            if (iqpt == 1):
                os.system('cp tmp/_ph0/'+prefix+'.dvscf1 save/'+prefix+'.dvscf_q' +
                          label)
                os.system('cp -r tmp/_ph0/'+prefix+'.phsave save/')
                if fc:
                    os.system('cp '+prefix+'.fc.xml save/ifc.q2r.xml')
